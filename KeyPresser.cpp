...
namespace {
    ...
    struct ToggleKey {
        ToggleKey(UINT _virtual_key = 0, clock_t _interval = 500) : virtual_key(_virtual_key), interval(_interval) {};
        UINT virtual_key;
        clock_t interval = 500;
        clock_t last_press = 0;
    };

    std::map<UINT, ToggleKey> keys;

    HWND window_handle;
    LPARAM GetKeypressLParam(UINT repeatCount, UINT scanCode, UINT extended, UINT context, UINT previousState, UINT transition) {
        return repeatCount
            | (scanCode << 16)
            | (extended << 24)
            | (context << 29)
    ...
    int GetKeyName(UINT virtual_key, wchar_t* out, int out_len) {
        UINT scanCode = MapVirtualKey(virtual_key, MAPVK_VK_TO_VSC);
        if (scanCode) {
            int res = GetKeyNameTextW(GetKeypressLParam(0,scanCode,0,0,0,0), out, out_len);
            if (res != 0)
                return res;
        }
        auto keyboardChar = MapVirtualKey(virtual_key, MAPVK_VK_TO_CHAR);
        if (keyboardChar) {
            out[0] = (wchar_t)keyboardChar;
            out[1] = 0;
            return 1;
        }
        return swprintf(out, out_len, L"%S", KeyName(virtual_key & 0x000000ff));
    }

    void AddToggleKey(UINT virtual_key, clock_t interval) {
        keys[virtual_key] = ToggleKey(virtual_key,interval);
        wchar_t* msg = new wchar_t[64];

        wchar_t key_name[32];
        GetKeyName(virtual_key, key_name, 32);
        swprintf(msg, 64, L"Keypress toggled ON for %s (interval %d ms)", key_name, interval);
        GW::GameThread::Enqueue([msg]() {
            GW::Chat::WriteChat(GW::Chat::CHANNEL_GWCA2, msg);
            delete[] msg;
            });
    }
    bool RemoveToggleKey(UINT virtual_key) {
        auto found = keys.find(virtual_key);
        if (found == keys.end())
            return false;
        keys.erase(virtual_key);
        wchar_t* msg = new wchar_t[64];
        wchar_t key_name[32];
        GetKeyName(virtual_key, key_name, 32);
        swprintf(msg, 64, L"Keypress toggled OFF for %s", key_name );
        GW::GameThread::Enqueue([msg]() {
            GW::Chat::WriteChat(GW::Chat::CHANNEL_GWCA2, msg);
            delete[] msg;
            });
    ...
    void ToggleKeyCmd(const wchar_t*, int argc, wchar_t** argv) {
        if (argc < 1)
            return;
        UINT virtual_key = 0;
        clock_t interval = 500;
        if (wcscmp(argv[1], L"clear") == 0 || wcscmp(argv[1], L"stop") == 0) {
            auto keys_copy = keys;
            for (auto& it : keys_copy) {
                RemoveToggleKey(it.first);
            }
            return;
        }
        if (wcscmp(argv[1], L"space") == 0) {
            virtual_key = VK_SPACE;
        }
        else if (wcscmp(argv[1], L"return") == 0 || wcscmp(argv[1], L"enter") == 0) {
            virtual_key = VK_RETURN;
        }
        else {
            if (wcslen(argv[1]) != 1)
                return;
            char keyboard_key_c = (char)(*(argv[1]));
            auto k = GetKeyboardLayout(0);
            virtual_key = VkKeyScanExA(keyboard_key_c, k);
        }
        if (RemoveToggleKey(virtual_key))
            return;

        if (argc > 2) {
            auto res = wcstoul(argv[2], NULL, 0);
            if (res != 0)
                interval = res;
        }
        AddToggleKey(virtual_key, interval);
...
void KeyPresser::Initialize(ImGuiContext* ctx, ImGuiAllocFns fns, HMODULE toolbox_dll)
{
    ToolboxPlugin::Initialize(ctx, fns, toolbox_dll);
    GW::Scanner::Initialize();
    GW::Initialize();

    GW::Chat::CreateCommand(L"togglekey", ToggleKeyCmd);
    GW::Chat::CreateCommand(L"keytoggle", ToggleKeyCmd);
    GW::UI::UIMessage ui_messages[] = {
        GW::UI::UIMessage::kMapChange,
        GW::UI::UIMessage::kLogout
    };
    for (auto& message_id : ui_messages) {
        GW::UI::RegisterUIMessageCallback(&ui_hook, message_id, OnUIMessage);
    }
    
    window_handle = GW::MemoryMgr::GetGWWindowHandle();

}
void KeyPresser::SignalTerminate() {
    GW::DisableHooks();
}
bool KeyPresser::CanTerminate() {
    return GW::HookBase::GetInHookCount() == 0;
}
void KeyPresser::Terminate()
{
    ToolboxPlugin::Terminate();
    GW::Terminate();
}
void KeyPresser::Update(float) {
    if (keys.empty() || GW::Chat::GetIsTyping())
        return;
    const auto now = clock();
    for (auto& it : keys) {
        auto& keypress = it.second;
        if (keypress.last_press + keypress.interval > now) {
            continue;
        }
        UINT scanCode = MapVirtualKey(keypress.virtual_key, MAPVK_VK_TO_VSC);
        if (!scanCode) {
            continue;
        }
        SendMessageA(window_handle, WM_KEYDOWN,keypress.virtual_key,GetKeypressLParam(0,scanCode,0,0,0,0));
        SendMessageA(window_handle, WM_KEYUP,keypress.virtual_key,GetKeypressLParam(1,scanCode,0,0,1,1));
        keypress.last_press = now;
    }
}
